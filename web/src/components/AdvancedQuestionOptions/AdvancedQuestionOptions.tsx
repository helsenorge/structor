...
const AdvancedQuestionOptions = (props: AdvancedQuestionOptionsProps): JSX.Element => {
    ...
    const getPlaceholder = props.item?.extension?.find((x) => x.url === IExtensionType.entryFormat)?.valueString ?? '';
    const getRepeatsText = props.item?.extension?.find((x) => x.url === IExtensionType.repeatstext)?.valueString ?? '';
    const minOccurs = props.item?.extension?.find((x) => x.url === IExtensionType.minOccurs)?.valueInteger;
    const maxOccurs = props.item?.extension?.find((x) => x.url === IExtensionType.maxOccurs)?.valueInteger;
        ...
        <>
                    ...
                    <InputField
                        ...
                        onBlur={(e) => {
                            if (e.target.value) {
                                ...
                                removeExtension(IExtensionType.entryFormat);
                            }
                        }}
                    />
                </FormField>
                                    ...
                                    <InputField
                                        ...
                                        onBlur={(e) => {
                                            if (e.target.value) {
                                                ...
                                                removeExtension(IExtensionType.repeatstext);
                                            }
                                        }}
                                    />
                                </FormField>
                                <div className="horizontal equal">
                                        ...
                                        <input
                                            ...
                                            onBlur={(event: React.ChangeEvent<HTMLInputElement>) => {
                                                if (!event.target.value) {
                                                    removeExtension(IExtensionType.minOccurs);
                                                } else {
                                                    const extension = {
                                    ...
                                    >
                                        <input
                                            ...
                                            onBlur={(event: React.ChangeEvent<HTMLInputElement>) => {
                                                if (!event.target.value) {
                                                    removeExtension(IExtensionType.maxOccurs);
                                                } else {
                                                    const extension = {
            ...
            <FormField label={t('Save capabilities')}>
                <RadioBtn
                    ...
                    checked={
                        props.item.extension?.find((ex) => ex.url === IExtensionType.saveCapability)?.valueCoding
                            ?.code ?? '0'
                    }
