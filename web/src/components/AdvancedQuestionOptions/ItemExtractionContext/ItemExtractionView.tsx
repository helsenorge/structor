...
const ItemExtractionContextView: FunctionComponent<ItemExtractionContextProps> = ({ item }: ItemExtractionContextProps) => {
    const { t } = useTranslation();
    const { dispatch } = useContext(TreeContext);

    const itemExtraction = item.extension?.find((ex) => ex.url === IExtensionType.itemExtractionContext)?.valueUri ?? '';
