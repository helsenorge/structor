...
export const importCSV = (csvData: string, qItems: Items, dispatch: React.Dispatch<ActionType>): void => {
    ...
    updateQuestionniareWithTranslation(parsedCsv.data as string[][], parsedCsv.data[0] as string[], qItems, dispatch);
}

const updateQuestionniareWithTranslation = (translatableItems: string[][], csvHeaders: string[], qItems: Items, dispatch: React.Dispatch<ActionType>) => {
...
const updateItemTranslation = (
    ...
    if (key.startsWith(TranslatableKeyProptey.item)) {
        ...
        if(key.includes(`.${TranslatableItemProperty.code}`)){
            returnValue = updateCodeTranslation(key, languageCode, text, dispatch, qItems);
        }
    }
    return returnValue;
}
